async function placeItem(bot, name, position) {
    ...
    if (!checkValidPosition(position, bot)){
        throw new Error("position for placeItem is out of environment. Perhaps you forgot to account for the offset.");
    }
    ...
    for (const vector of faceVectors) {
        ...
        if (block?.name !== "air") {
            ...
            faceVector = vector;
            await think(bot, `Placing ${name} on ${block.name} at ${block.position}`);
            break;
        }
    }
    if (!referenceBlock) {
        await think(
            bot, `No block to place ${name} on. You cannot place a floating block.`
        );
        return;
    }
    ...
    try {
        ...
        await think(bot, `Placed ${name}`);
    } catch (err) {
        const item = bot.inventory.findInventoryItem(itemByName.id);
        if (item?.count === item_count) {
            await think(
                bot, `Error placing ${name}: ${err.message}, please find another position to place`
            );
        } else {
            await think(bot, `Placed ${name}`);
        }
