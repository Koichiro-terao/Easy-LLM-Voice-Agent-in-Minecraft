...
function resolveMcHost(inputHost) {
    const host = inputHost || "localhost";
...
async function generate_init_state(){
    const mcHost = resolveMcHost(await input("Enter host name of Minecraft server [localhost] > "));
    const mcPort = parseInt(await input("Enter port number of Minecraft server [25565] > ")) || 25565;

    const destFile = await input(`Enter filepath of output file [world_config.json] > `) || "world_config.json";

    const corner1 = parseVec3Input(await input(`Enter the coordinates of one corner of the world. Example: -10 -50 -10 > `));
    if(!corner1){
        console.log("Three numbers have to be entered.");
        process.exit(1);
    }
    const corner2 = parseVec3Input(await input(`Enter the coordinates of opposite corner of the world. Example: 10 -40 10 > `));
    if(!corner2){
        console.log("Three numbers have to be entered.");
        process.exit(1);
    }
    ...
    await input(`envBox ${JSON.stringify(envBox)} will be written into the output file. Press ENTER to continue.`)
    ...
    bot.once('spawn', async () => {
        ...
        try {
            ...
            while(true){
                ...
                const agentName = await input(`Position is ${pos}. Enter the agent name whose initial position is here > `);
                players[agentName] = {
                    type: "BotPlayer"
                };
                status[agentName] = {
                ...
                while(!['y','n'].includes(ans)){
                    ans = (await input(`${agentName}'s initial position is recorded. Do you add another agent? ([y]/n) > `)) || "y";
                }
                if(ans === "n"){
                    break;
                }
            ...
            console.log(`Complete! World config data has been written to ${destFile}`);
