...
let physics_interval_ms = 50
let physics_timestamp = physics_interval_ms / 1000 // 0.05
...
function inject (bot, { physicsEnabled, maxCatchupTicks }) {
  ...
  function doPhysics () {
    ...
    catchupTicks = 0
    while (timeAccumulator >= physics_timestamp) {
      tickPhysics(now)
      timeAccumulator -= physics_timestamp
      catchupTicks++
  ...
  function updatePosition (now) {
    ...
    const maxDeltaYaw = physics_timestamp * physics.yawSpeed
    const maxDeltaPitch = physics_timestamp * physics.pitchSpeed
  ...
  bot.updateTickRate = function (tickRate) {
    cleanup()
    physics_interval_ms = 1000 / tickRate
    physics_timestamp = physics_interval_ms / 1000
